...
def _compute_area(geometry: dict) -> float:
    """Rough area estimate for z-index sorting (smaller on top)."""
    try:
...
def _generate_html(
    ...
    *,
    ...
    # Compute area for each feature and sort: larger first (drawn behind), smaller last (drawn on top)
    indexed_features = []
    for idx, feat in enumerate(raw_features):
        area = _compute_area(feat.get("geometry", {}))
        indexed_features.append((area, idx, feat))
    indexed_features.sort(key=lambda x: x[0], reverse=True)

    # Build metadata in display order (but keep original indices for referencing)
    fields_meta = []
    display_order = []
    for display_idx, (area, orig_idx, feat) in enumerate(indexed_features):
        ...
        fields_meta.append({
            ...
            "field_id": props.get("field_id", f"field-{orig_idx}"),
            ...
            "lat": coords[0],
            "lon": coords[1],
            "color": _FIELD_COLORS[orig_idx % len(_FIELD_COLORS)],
        })
        display_order.append(orig_idx)

    # Reorder features in geojson to match display order (larger behind)
    reordered_features = [feat for _area, _idx, feat in indexed_features]
    geojson_data["features"] = reordered_features
...
<style>
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  .control-label {{ font-size: 0.8rem; color: #495057; font-weight: 500; }}
  .opacity-slider {{
    width: 120px;
    height: 4px;
  ...
  .toggle-btn:hover {{ background: #E8F5E9; }}
  #field-list {{
    flex: 1;
    overflow-y: auto;
    padding: 8px;
  ...
  .field-item.active {{ background: #C8E6C9; border-color: #66BB6A; }}
  ...
  <div id="sidebar">
    ...
    <div id="controls">
      <div class="control-row">
        ...
        <input type="range" class="opacity-slider" id="opacitySlider" min="0" max="100" value="15">
      </div>
      <div class="control-row">
...
<script>
  ...
  function getStyle(idx, isHover) {{
    var meta = fieldMeta[idx] || {{}};
    var color = meta.color || '#4E79A7';
    var isActive = (activeFieldIndex === idx);
    var isDimmed = (activeFieldIndex !== null && activeFieldIndex !== idx);
    ...
    var weight = isHover || isActive ? 3.0 : 1.5;
    var opacity = isDimmed ? 0.3 : (isHover || isActive ? 1.0 : 0.8);
    var fillOpacity = isDimmed ? 0.05 : (isHover || isActive ? baseFillOpacity + 0.15 : baseFillOpacity);
  ...
  var geoLayer = L.geoJSON(geojsonData, {{
    ...
    onEachFeature: function(feature, layer) {{
      ...
      layer.on('mouseover', function() {{
        layer.setStyle(getStyle(idx, true));
        highlightSidebarItem(idx);
      }});
      layer.on('mouseout', function() {{
        layer.setStyle(getStyle(idx, false));
        unhighlightSidebarItem(idx);
      }});
      ...
      var label = L.marker(center, {{
        icon: L.divIcon({{
          ...
          html: '<div class="field-label" style="border-color:' + meta.color + ';">' + meta.field_id + '</div>',
          iconSize: null,
          iconAnchor: [30, 10]
        }}),
        interactive: false
  ...
  fieldMeta.forEach(function(meta, idx) {{
    ...
    item.innerHTML =
      '<input type="checkbox" class="field-checkbox" checked data-idx="' + idx + '">' +
      '<div class="field-color" style="background:' + meta.color + '"></div>' +
    ...
    // Checkbox toggle
    var checkbox = item.querySelector('.field-checkbox');
    checkbox.addEventListener('change', function(e) {{
      e.stopPropagation();
      toggleFieldVisibility(idx, checkbox.checked);
    }});
  ...
  function highlightSidebarItem(idx) {{
    var items = listEl.querySelectorAll('.field-item');
    if (items[idx]) items[idx].classList.add('active');
  }}

  function unhighlightSidebarItem(idx) {{
    var items = listEl.querySelectorAll('.field-item');
  ...
  function setActiveField(idx) {{
    ...
    layers.forEach(function(layer, i) {{
      layer.setStyle(getStyle(i, false));
    }});
    // Highlight sidebar
    var items = listEl.querySelectorAll('.field-item');
    items.forEach(function(itm, i) {{
      itm.classList.toggle('active', i === idx);
    }});
  }}

  function toggleFieldVisibility(idx, visible) {{
    fieldMeta[idx].hidden = !visible;
    var layer = layers[idx];
    if (layer) {{
      layer.setStyle(getStyle(idx, false));
      if (visible) {{
    ...
    // Toggle label
    labelMarkers.forEach(function(lm) {{
      if (lm.idx === idx) {{
        if (visible) {{
          lm.marker.addTo(map);
        }} else {{
    ...
    var items = listEl.querySelectorAll('.field-item');
    if (items[idx]) items[idx].classList.toggle('hidden-field', !visible);
  }}

  // Opacity slider
  var opacitySlider = document.getElementById('opacitySlider');
  opacitySlider.addEventListener('input', function() {{
    baseFillOpacity = parseInt(this.value) / 100;
    layers.forEach(function(layer, i) {{
      layer.setStyle(getStyle(i, false));
    }});
  }});
  ...
  toggleAllBtn.addEventListener('click', function() {{
    ...
    fieldMeta.forEach(function(meta, idx) {{
      var checkbox = listEl.querySelector('.field-checkbox[data-idx="' + idx + '"]');
      if (checkbox) checkbox.checked = allVisible;
      toggleFieldVisibility(idx, allVisible);
    }});
  }});
